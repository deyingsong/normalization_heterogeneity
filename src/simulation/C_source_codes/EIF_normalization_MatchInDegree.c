...
// InDegree variant (refactored).
// NOTE: Without the original specialized logic here, this wrapper currently matches the baseline computations.
// If the original file scaled weights by in-degree or altered connectivity semantics, adjust inside eif_simulate_core or add a hook.
// Signature: [s, Isyn_record, v_record] = EIF1DRFfastslowSynAtttSpatRecInDegree(sx, Wrf, Wrr, params)
